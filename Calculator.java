...
public class Calculator {
    ...
    public String result(String exp) throws CalcException {
        ...
        if (expItems.size() != 3) {
            throw new CalcException("ОШИБКА. Выражение должно иметь вид: \"Число1 Операция Число2\", разделены пробелом...");
        }
        ...
        if (isRomanExp){
            if (res <= 0){
                throw new CalcException("ОШИБКА. Недопустимый результат для римских чисел");
            }
        }
        if (isRomanExp) {
            String sign = res < 0 ? "-" : "";
